...
from transformers import AutoTokenizer, GPT2Config, LlamaConfig, CLIPVisionConfig, LlavaConfig, AutoConfig, SiglipVisionConfig
...
def convert_checkpoint_from_transformers_to_megatron(args):
    ...
    if args.vision == 'clip':
        vision_state_dict = torch.load('/path/to/clip-vit-large-patch14-336/pytorch_model.bin', map_location="cpu")
        vision_state_dict = {k: v for k, v in vision_state_dict.items() if not k.startswith('text')}
        mm_projector_state_dict = torch.load('/path/to/weboutput_llava/checkpoint/llava-pretrain-llama3-70B-lr-1e-3-bs-1-seqlen-2048-pr-bf16-tp-8-pp-1-ac-sel-do-false-sp-true-70B/iter_0002200/mm_projector.pt', map_location="cpu")
        vision_config = CLIPVisionConfig.from_pretrained('/path/to/clip-vit-large-patch14-336/')
    elif args.vision == 'siglip':
        mm_projector_state_dict = torch.load('/path/to/clip-vit-large-patch14-336/mm_projector_to_8B_siglip.pth', map_location="cpu")
        
        siglip = SiglipModel.from_pretrained('/path/to/siglip-so400m-patch14-384')
        siglip_config = AutoConfig.from_pretrained('/path/to/siglip-so400m-patch14-384')
        ...
        #print (list(vision_state_dict.keys()))
    else:
        print ('vision model not supported')
        exit()
    # print (list(vision_state_dict.keys()))
    # print (list(mm_projector_state_dict.keys()))
    ...
    for layer_id in range(config.num_hidden_layers):
        ...
        if layer_id < vision_config.num_hidden_layers:
            q_vision_weight = vision_state_dict['vision_model.encoder.layers.' + str(layer_id) + '.self_attn.q_proj.weight']
            k_vision_weight = vision_state_dict['vision_model.encoder.layers.' + str(layer_id) + '.self_attn.k_proj.weight']
            v_vision_weight = vision_state_dict['vision_model.encoder.layers.' + str(layer_id) + '.self_attn.v_proj.weight']
    ...
    if args.vision == 'clip':
        ...
        internal_state_dict['vision_transformer.patch_embedding.bias'] = vision_state_dict['vision_model.embeddings.patch_embedding.bias']
        internal_state_dict['vision_transformer.post_layernorm.weight'] = vision_state_dict['vision_model.post_layernorm.weight']
        internal_state_dict['vision_transformer.post_layernorm.bias'] = vision_state_dict['vision_model.post_layernorm.bias']
...
def convert_checkpoint_from_megatron_to_transformers(args):
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        '/path/to/llama-3/hf/Meta-Llama-3-70B-Instruct/',
    ...
    if args.vision == 'clip':
        ...
        vision_config = SiglipVisionConfig.from_pretrained('/path/to/models/siglip-so400m-patch14-384')
    llava_config = LlavaConfig(text_config=config, vision_config=vision_config, image_token_index=128200, architectures=["LlavaForConditionalGeneration"])
        ...
        if pp_rank == 0:
            ...
            for key, val in get_element_from_dict_by_path(tp_state_dicts[0], 'model.language_model.vision_tower').items():
                ...
                if key == "vision_tower.conv1.weight":
                    ...
                    output_state_dict["vision_tower.vision_model.embeddings.patch_embedding.bias"] = val.to(dtype).clone()
                    ...
                    'mlp.linear_fc1.weight', 'mlp.linear_fc1.bias', 'mlp.linear_fc2.weight']]):
                        ...
                        params = torch.cat(
                            [val]
                            + [
                                get_element_from_dict_by_path(tp_state_dicts[tp_rank], 'model.language_model.vision_tower')[key.replace('vision_tower.', '') if args.vision == 'siglip' else key]
                                for tp_rank in range(1, tp_size)
                            ],
                            dim=dim,
                        ).to(dtype)
