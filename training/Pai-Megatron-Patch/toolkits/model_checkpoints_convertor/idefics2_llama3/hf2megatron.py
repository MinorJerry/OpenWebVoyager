...
from transformers import AutoTokenizer, GPT2Config, MistralConfig, LlamaForCausalLM, AutoConfig, Idefics2Processor, AutoImageProcessor
...
def add_checkpointing_args(parser):
    ...
    parser.add_argument(
        "--adapt_position_embedding",
        action="store_true",
        help=(
            "If True, interpolate position embedding, else, convert checkpoints. "
        ),
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="Path to the perceiver.",
    )

    parser.add_argument(
        "--num_perceiver_attention_heads",
        type=int,
        required=True,
        default=16,
        help="num of perceiver_attention_heads",
    )
    parser.add_argument(
        "--old_max_nb_patches_h",
        type=int,
        required=False,
        default=27,
        help="num of perceiver_attention_heads",
...
def convert_checkpoint_from_transformers_to_megatron(args):
    ...
    # configs
    text_config = LlamaConfig.from_pretrained(args.llm_path)
    # idefics2_config = Idefics2Config.from_pretrained(args.idefics2_path)
    vision_config = AutoConfig.from_pretrained(args.vision_path)
    vision_config = Idefics2VisionConfig(
        hidden_size=vision_config.vision_config.hidden_size,
        intermediate_size=vision_config.vision_config.intermediate_size,
        num_hidden_layers=vision_config.vision_config.num_hidden_layers,
        num_attention_heads=vision_config.vision_config.num_attention_heads,
        image_size=vision_config.vision_config.image_size,
        patch_size=vision_config.vision_config.patch_size,
        hidden_act="gelu_pytorch_tanh",
    )

    # Perceiver can be from scratch
    if args.perceiver_path is not None:
        perceiver_config = Idefics2PerceiverConfig.from_pretrained(args.perceiver_path)
        # FIXME: 
        # because the original num_kv_heads is not divisilbe by GPU_NUM (8), multiply it by 4
        # hard-coded for now. 

        # the vanilla setting of idefics2-8b
        if perceiver_config.num_key_value_heads == 4 and args.target_tensor_model_parallel_size > 4:
            perceiver_config.num_key_value_heads *= 4
    else:
        perceiver_config = Idefics2PerceiverConfig(num_key_value_heads=args.num_perceiver_attention_heads)  # default is 16

    assert perceiver_config.num_key_value_heads % args.target_tensor_model_parallel_size == 0, "num_key_value_heads must be divisible by target_tensor_model_parallel_size"
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        ...
        additional_special_tokens=['<fake_token_around_image>', '<image>', '<end_of_utterance>']
    )

    if tokenizer.pad_token is None:
        tokenizer.add_special_tokens(special_tokens_dict=dict(pad_token="<unk>"))

    image_processor = Idefics2ImageProcessor.from_pretrained(args.vision_path)
    image_processor.resample = 2
    image_processor.size['longest_edge'] = image_processor.size.pop("height")
    image_processor.size['shortest_edge'] = image_processor.size.pop("width")
    processor = Idefics2Processor(image_processor, tokenizer = tokenizer, image_seq_len=perceiver_config.resampler_n_latents)
    processor.save_pretrained(args.save_path)

    ## Config

    idefics2_config = Idefics2Config(image_token_id=tokenizer.convert_tokens_to_ids("<image>"), 
                                     vision_config=vision_config, 
                                     perceiver_config=perceiver_config, 
                                     text_config=text_config)
    
    idefics2_config.save_pretrained(args.save_path)
    
    # checkpoints

    sub_dirs = sorted([x for x in os.listdir(args.llm_path) if x.endswith(".safetensors")])
    state_dict = {}
    for checkpoint_name in sub_dirs:
        current_chunk = load_file(os.path.join(args.llm_path, checkpoint_name), 'cpu')
        state_dict.update(current_chunk)
    ...
    for layer_id in range(text_config.num_hidden_layers):
        ...
        try:
            ...
            print ('try failed, setting the rope theta to', text_config.rope_theta)
    ...
    # vision
    vision_model = SiglipModel.from_pretrained(args.vision_path)
    vision_state_dict = vision_model.vision_model.state_dict()
    ...
    if args.perceiver_path is not None:
        perceiver_state_dict = torch.load( os.path.join(args.perceiver_path, "pytorch_model.bin") )
    else:
        perceiver_blank_model = Idefics2Connector(idefics2_config)
        perceiver_state_dict = perceiver_blank_model.state_dict()

    # 0. latent: 64 x hidden_size nn.Params

    num_attn_head_q = perceiver_config.resampler_n_heads # 16
    num_attn_head_kv = perceiver_config.num_key_value_heads # 4 * 4 = 16
    num_repeat = num_attn_head_q//num_attn_head_kv # 4

    # if there's a conflict between 
    assert text_config.hidden_size == text_config.hidden_size, "the size of perceiver output layer doesn't match the size of the LLM. Need to re-initialize the weights."
    ...
    for pp_rank in range(args.target_pipeline_model_parallel_size):
        ...
        if pp_rank == 0:
            ...
            ## Perceiver
            for layer in range(perceiver_config.resampler_depth):
                assert pp_rank == 0
                perceiver_layers_to_copy = [
                    layer_name
                    for layer_name in state_dict.keys()
                    if layer_name.startswith(f"perceiver_transformer.layers.{layer}.")
                ]
...
def convert_checkpoint_from_megatron_to_transformers(args):
    ...
    # try:
        ...
        processor = Idefics2Processor.from_pretrained(args.config_path)
    else:
    # except:
        print('loading config from', config_path)
        config = AutoConfig.from_pretrained(config_path)
        processor = Idefics2Processor.from_pretrained(config_path)

    config.text_config.vocab_size = vocab_size
    config.perceiver_config.num_key_value_heads = args.num_perceiver_attention_heads
    ...
    output_state_dict["lm_head.weight"] = params.to(dtype).clone()[:vocab_size, :]
    ...
    rm_keys = []
    for name, params in output_state_dict.items():
        if name.endswith("inv_freq"):
            rm_keys.append(name)
    
    for name in rm_keys:
        output_state_dict.pop(name)
...
def interpolate_position_embeddings(position_embeddings, old_max_nb_patches_h, old_max_nb_patches_w,  new_max_nb_patches_h, new_max_nb_patches_w):
    # Get the original dimensions
    old_seq_length, visual_hidden_size = position_embeddings.shape

    # Reshape the position embeddings to a 2D grid
    position_embeddings_grid = position_embeddings.view(old_max_nb_patches_h, old_max_nb_patches_w, visual_hidden_size).unsqueeze(0)
    position_embeddings_grid = position_embeddings_grid.permute(0, 3, 1, 2)

    # Apply 2D bilinear interpolation to resize the position embeddings grid
    new_position_embeddings_grid = F.interpolate(position_embeddings_grid, size=(new_max_nb_patches_h, new_max_nb_patches_w), mode='bilinear', align_corners=True).squeeze(0)
    new_position_embeddings_grid = new_position_embeddings_grid.permute(1, 2, 0)

    # Reshape the new position embeddings grid back to the original format
    new_position_embeddings = new_position_embeddings_grid.view(new_max_nb_patches_h * new_max_nb_patches_w, visual_hidden_size)

    return new_position_embeddings



def interpolate_vit_position_embedding(args):
    """
    When changing the image resolution size, need to resize and interpolate the position embedding.
    ...
    for pp_rank in range(pp_size):
        ...
        if pp_rank == 0:
            ...
            for tp in range(tp_size):
                vision_enc_dict = get_element_from_dict_by_path(tp_state_dicts[tp], "model.language_model.vision_model")
                vision_enc_dict['position_embeddings.weight'] = interpolate_position_embeddings(vision_enc_dict['position_embeddings.weight'], 
                                                                old_max_nb_patches_h=args.old_max_nb_patches_h, 
                                                                old_max_nb_patches_w=args.old_max_nb_patches_w,
                                                                new_max_nb_patches_h=args.new_max_nb_patches_h,
                                                                new_max_nb_patches_w=args.new_max_nb_patches_w,
                                                                )
        # saving the state dict as per the tp_rank and pp_rank
        for tp_rank in range(args.target_tensor_model_parallel_size):
            tp_state_dicts[tp_rank]["checkpoint_version"] = 3.0
            tp_state_dicts[tp_rank]["args"] = megatron_args
            checkpoint_dir = (
                f"mp_rank_{tp_rank:02d}"
                if args.target_pipeline_model_parallel_size == 1
                else f"mp_rank_{tp_rank:02d}_{pp_rank:03d}"
            ...
            if args.print_checkpoint_structure:
                ...
                recursive_print(None, tp_state_dicts[tp_rank])
    ...
    # copy config and processor

    config = AutoConfig.from_pretrained(args.config_path)
    config.vision_config.image_size = args.image_size # int(args.new_max_nb_patches_h/args.old_max_nb_patches_h * config.vision_config.image_size)
    config.save_pretrained(args.save_path)
...
def main():
    ...
    if args.adapt_position_embedding:
        interpolate_vit_position_embedding(args)
    else:
