...
from prompts import SYSTEM_PROMPT_ACTREE_IMAGE
...
from evaluation.auto_eval import auto_eval_by_gpt4v
...
def main():
    ...
    parser.add_argument("--api_eval_model", default="", type=str, help="evaluation API")
    parser.add_argument("--max_rerun_times", default=1, type=int)
    ...
    max_rerun_times = 1
    if args.api_eval_model:
        client_eval = OpenAI(api_key=args.api_key)
        max_rerun_times = args.max_rerun_times
    ...
    for task_id in range(len(tasks)):
        for rerun in range(max_rerun_times):
            task = tasks[task_id]
            task_dir = os.path.join(result_dir, 'task{}--run{}'.format(task["id"], rerun))
            if os.path.exists(os.path.join(task_dir, 'interact_messages.json')):
                print ('This task has been processed', task["id"])
                continue
            os.makedirs(task_dir, exist_ok=True)
            setup_logger(task_dir)
            logging.info(f'########## TASK{task["id"]}--run{rerun} ##########')
            print(f'########## TASK{task["id"]}--run{rerun} ##########')
            ...
            # init_msg_text = init_msg + obs_prompt_text
            ...
            auto_eval_res = auto_eval_by_gpt4v(task_dir, client_eval, args.api_eval_model, 3)
            if auto_eval_res:
                with open(os.path.join(task_dir, 'eval_res.json'), 'w') as fw:
                    json.dump(auto_eval_res, fw)
                    
            if auto_eval_res == 0:
                continue
            elif auto_eval_res[-1]['success'] == None:
                continue
            elif auto_eval_res[-1]['success'] == 0:
                continue
            else:
                print(f'{task_dir} Success in rerun {rerun}\n')
                break
